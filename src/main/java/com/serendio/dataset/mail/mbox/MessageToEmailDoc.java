...
public class MessageToEmailDoc {
	...
	public String getNameFromHeader(Message message) {
		...
		if (message.getHeader().getField("From") != null) {
			String listName[] = message.getHeader().getField("From").getBody()
					.split("<");
	...
	// TODO: Keep consistent pattern of space between operators. like either
	// keep x=5 or x = 5.
	...
	public EmailDoc messageToemailDoc(InputStream messageBytes)
	...
	private EmailDoc parseBodyParts(Multipart multipart, EmailDoc emailObject)
			throws IOException {
		List<Entity> list = multipart.getBodyParts();
		for (Entity e : list) {
			String s = e.getMimeType();
			if (s.equals("text/plain")) {
				emailObject.setContent(emailObject.getContent()
						+ getTextPart(e));
			}
			...
			if (e.getDispositionType() != null) {
				emailObject.setAttachment(emailObject.getAttachment()
						+ e.getFilename());
			}
		}
		return emailObject;
