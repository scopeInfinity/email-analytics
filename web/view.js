  ...
  var that = {
    ...
    redraw:function(){
      ...
      particleSystem.eachEdge(function(edge, pt1, pt2){
        ...
        var sentiments = edge.data.properties['sentiments'];
        var count = sentiments.length;

        ctx.lineWidth = 1;//(count+maxCount-1)/maxCount;
        ctx.beginPath();
        canvas_arrow(ctx,pt1.x,pt1.y,pt2.x,pt2.y,sentiments,(count+maxCount-1)/maxCount);
...
function refreshCentralGraph() {
  var parent = $('#viewport').parent();
  $('#viewport').remove();
  parent.append('<canvas id="viewport" width="1024" height="768" style=" display: block; " tabindex="2"></canvas>');
  ...
  console.log("Refresh Graph Done!");

  return sys;
}
function fetchCentralGraph(sys,email){
  // Calling `data.php` for obtaning Graph
  console.log("Fetching Data");
  jQuery.ajax({
      type: "POST",
      url: 'data.php?op=users',
      data: { 'email': email },
      success: function(data) {
                ...
                $.each(pushedData['edges'], function(i, destinations)
                {
                    $.each(destinations, function(j, property)
                    {
                        ...
                        if(maxCount<property['sentiments'].length)
                          maxCount = property['sentiments'].length;
                    }) 
                });
